...
interface GlitchTextProps {
  ...
  className?: string;
  /** Glow color accent — defaults to cyan */
  glowColor?: "cyan" | "green" | "amber" | "red";
  /** Enable continuous glitch animation — defaults to false (hover-only) */
  continuous?: boolean;
}
...
export function GlitchText({
  ...
  return (
    ...
    >
      {children}
      {/* Before pseudo-layer for glitch offset (CSS handles this) */}
      <span
        className="glitch-layer glitch-layer--before pointer-events-none absolute inset-0 select-none"
        aria-hidden="true"
        data-text={children}
      />
      {/* After pseudo-layer for opposite glitch offset */}
      <span
